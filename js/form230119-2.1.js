...
const buildForm = (gptQuestions) => {
  ...
  for (const section in gptQuestions) {
    var innerCounter = 0;
    //console.log(section,gptQuestions[section]);
    $("#questionnaire").append(`<h3 id="H3s${outerCounter}">${section}</h3>`);
    for (q in gptQuestions[section]) {
      innerCounter++;
      // if (counter>2) continue;
      if (gptQuestions[section][q][3] === null) { // (1)
        $("#questionnaire").append(fillTemplate(gptQuestions[section][q][1],outerCounter,q));
      } else {
        let fieldsetId = `fs${outerCounter}-${innerCounter}`;
        $("#questionnaire").append(`<fieldset class="f04" id="${fieldsetId}">
         <legend id="qu${outerCounter}">${gptQuestions[section][q][1]}?</legend></fieldset>`);
         for (let lbl=0;lbl<gptQuestions[section][q][3].length;lbl++) {
           $(`#${fieldsetId}`).append(`<label class="form-control"><input type="radio" name="s${outerCounter}q${q}" value="${lbl+1}"> ${gptQuestions[section][q][3][lbl]}</label>`);
         }
      }
    }
    outerCounter++;
...
const columns = 'Nr,Siglum,Area,Question,Subq'.split(',');

$(document).ready(function () {
  $.getJSON( "webforms/fragebogen-1.json", function( data ) {
    buildForm(data);
    $("#result").hide();
